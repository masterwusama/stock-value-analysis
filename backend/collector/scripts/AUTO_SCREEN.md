...
| `--pb-min` / `--pb-max` | 仅 market 源预筛：PB 区间（默认 0.01~1.40）。施洛斯买点 = 0.75×每股净资产，只跑 `--buy schloss` 时把上限压到 0.75 可少扫一大半 |
...
| 命中 0 但感觉不该为空 | 施洛斯买点 = 0.75×每股净资产，即要求 PB ≤ 0.75；只是破净（PB<1）而未到七五折的股票不会命中，属正常 |
