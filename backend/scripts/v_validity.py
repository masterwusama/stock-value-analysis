...
- `v_facts` 当时补了两个权益键名（A 股银行系的归母权益「归属于母公司股东的权益」、港股的
  总权益「股东权益合计」）。当时核心列映射两个都不认，导致 98 家（A 42＋港 56，实测全是金融）
  整条权益链在回测里取不到、V 的 40 分凭空判不动；当时在读取端从 `extras` 兜底。
...
- **补上权益键名后，金融股是「排在 V 顶端」而不是「缺分」**：实测 65 家 A 股金融类 V 中位 75.2
  ...
  权益读取现统一为归母优先、总权益兜底；生产计算读原始报表行，回测优先读 `extras`，
  无可用权益才回退核心列。核心列别名映射已同步，既有派生字段另行核实、备份后定向修复。
...
def v_facts(g, av, sid):
    ...
    权益由 `fy_facts` 按共享口径读取；V 专用的负债、无形资产、负债率在这里补齐。
    """
    f = fy_facts(g, av, {}, sid)
